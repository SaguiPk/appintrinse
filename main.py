...
class MainApp(MDApp):
    ...
    def verificador(self, nome_paciente, horario_paciente):
        ...
        print(self.txt_input_hora, type(self.txt_input_hora))
        ...
        if self.txt_input_nome != '' and self.txt_input_hora != None and isinstance(self.txt_input_nome, str):
            ...
            if encontrou is False:
                ...
                if encontrou is False:
                    print(f'não enconrou no dia de hoje, verificar na agenda toda')
                    for dia in lista_dias:
                        ...
                        for i, pessoa in enumerate(self.agenda_psico[f'{dia}']):
                            ...
                            if pd.notna(pessoa) and self.paciente.upper() in n:
                                ...
                                id_msg.text = (f'[size=70]Bem Vindo {n.title()}[/size]\n\n'
                                               f'[size=60]Psicóloga(o) avisada[/size]\n'
                                               f'[size=50]Sua sessão está agendada para {dia.lower()} as {horario_pessoa}[/size]\n'
                                               f'[size=50]Consulte a recepção![/size]')
